...
static void handle_read(uint8_t num)
{
  ...
  ESP_LOGI(TAG, "handle_read msg=%s", msg);
  switch (clients[num].last_opcode)
  {
  case WEBSOCKET_OPCODE_CONT:
    break;
  case WEBSOCKET_OPCODE_BIN:
    clients[num].scallback(num, WEBSOCKET_BIN, msg, header.length, clients[num].scallback_arg);
    break;
  case WEBSOCKET_OPCODE_TEXT:
    clients[num].scallback(num, WEBSOCKET_TEXT, msg, header.length, clients[num].scallback_arg);
    break;
  case WEBSOCKET_OPCODE_PING:
    ws_send(&clients[num], WEBSOCKET_OPCODE_PONG, msg, header.length, 0);
    clients[num].scallback(num, WEBSOCKET_PING, msg, header.length, clients[num].scallback_arg);
    break;
  case WEBSOCKET_OPCODE_PONG:
    if (clients[num].ping)
    {
      clients[num].scallback(num, WEBSOCKET_PONG, NULL, 0, clients[num].scallback_arg);
      clients[num].ping = 0;
    }
    break;
  case WEBSOCKET_OPCODE_CLOSE:
    clients[num].scallback(num, WEBSOCKET_DISCONNECT_EXTERNAL, NULL, 0, clients[num].scallback_arg);
                                  ...
                                  void *pvParameters)
{
  ...
  for (int i = 0; i < WEBSOCKET_SERVER_MAX_CLIENTS; i++)
  {
    if (clients[i].conn)
      continue;
    clients[i] = ws_connect_client(conn, url, NULL, NULL, callback, pvParameters);
    callback(i, WEBSOCKET_CONNECT, NULL, 0, pvParameters);
    if (!ws_is_connected(clients[i]))
    {
      callback(i, WEBSOCKET_DISCONNECT_ERROR, NULL, 0, pvParameters);
                         ...
                         void *pvParameters)
{
  ...
  return ws_server_add_client_protocol(conn, msg, len, url, NULL, callback, pvParameters);
}
...
int ws_server_remove_client(int num)
{
  ...
  {
    clients[num].scallback(num, WEBSOCKET_DISCONNECT_INTERNAL, NULL, 0, clients[num].scallback_arg);
    ws_disconnect_client(&clients[num], 0);
    ret = 1;
  }
  xSemaphoreGive(xwebsocket_mutex);
...
int ws_server_remove_clients(char *url)
{
  ...
  for (int i = 0; i < WEBSOCKET_SERVER_MAX_CLIENTS; i++)
  {
    ...
    {
      clients[i].scallback(i, WEBSOCKET_DISCONNECT_INTERNAL, NULL, 0, clients[i].scallback_arg);
      ws_disconnect_client(&clients[i], 0);
      ret += 1;
    }
  }
...
int ws_server_send_text_client_from_callback(int num, char *msg, uint64_t len)
{
  ...
  if (ws_is_connected(clients[num]))
  {
    ...
    if (err)
    {
      clients[num].scallback(num, WEBSOCKET_DISCONNECT_ERROR, NULL, 0, clients[num].scallback_arg);
      ws_disconnect_client(&clients[num], 0);
      ret = 0;
    }
  }
...
int ws_server_send_text_clients_from_callback(char *url, char *msg, uint64_t len)
{
  ...
  for (int i = 0; i < WEBSOCKET_SERVER_MAX_CLIENTS; i++)
  {
    if (clients[i].url != NULL && ws_is_connected(clients[i]) && !strcmp(clients[i].url, url))
    {
      ...
      {
        clients[i].scallback(i, WEBSOCKET_DISCONNECT_ERROR, NULL, 0, clients[i].scallback_arg);
        ws_disconnect_client(&clients[i], 0);
      }
    }
  }
