                              ...
                              void *scallback_arg)
{
  ...
  client.ccallback_arg = ccallback_arg;
  client.scallback = scallback;
  client.scallback_arg = scallback_arg;
  return client;
}
...
char *ws_read(ws_client_t *client, ws_header_t *header)
{
  ...
  { // LEN = 127
    header->length = (uint64_t)buf[2] << 56 | (uint64_t)buf[3] << 48 | (uint64_t)buf[4] << 40 | (uint64_t)buf[5] << 32 | (uint64_t)buf[6] << 24 | (uint64_t)buf[7] << 16 | (uint64_t)buf[8] << 8 | (uint64_t)buf[9];
    pos = 10;
  }
